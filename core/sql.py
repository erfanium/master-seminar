...
def get_histories(conn: sqlite3.Connection, table: str, table_id: int) -> dict:
    ...
    for rec in conn.execute(
        f"SELECT * FROM `history` WHERE `table`=? AND `table_rowid` = ? ",
        (table, table_id),
    ):
        yield (dict(rec))
...
def insert_selection_from_samples(
    ...
    ids = ",".join(
        [
            str(dict(rec)["id"])
            for rec in conn.execute(
                f"SELECT id FROM samples WHERE name in {samples_clause}"
            )
        ]
    )
    ...
    if query_in_db != query or force:
        ...
        return insert_selection_from_sql(
            conn=conn, query=query, name=name, description=description
        )
    else:
...
def create_variants_indexes(conn, indexed_fields={"pos", "ref", "alt"}):
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS `idx_genotypes` ON genotypes (`variant_id`)"
    )
...
DEFAULT_SELECTION_NAME = "All Variants"  # adjust if needed
...
def insert_variants(
    ...
    """
    Insert many variants into the database with optimized bulk insert for genotypes.

    Args:
        conn (sqlite3.Connection): SQLite connection
        variants (List[dict]): list of variant dictionaries
        total_variant_count (int, optional): total number of variants (for progress)
        progress_every (int, optional): frequency to report progress
        progress_callback (Callable, optional): callback for progress messages
    """
    variants_local_fields = get_table_columns(conn, "variants")
    annotations_local_fields = get_table_columns(conn, "annotations")
    samples_local_fields = get_table_columns(conn, "genotypes")

    # Map sample name -> sample id
    ...
    # Buffer for batch genotype insert
    genotype_buffer = []
    GENOTYPE_BATCH_SIZE = 5000
    genotype_fields_batch = None  # Tracks which fields are used for current batch

    def _bulk_insert_genotypes(cursor, buffer, insert_fields):
        """Helper function to execute bulk insert/update for genotypes"""
        if not buffer:
            return
        query_fields = ",".join(f"`{f}`" for f in insert_fields)
        placeholders = ",".join("?" for _ in insert_fields)
        update_clause = ",".join(f"{f}=excluded.{f}" for f in insert_fields)
        query = f"""
            INSERT INTO genotypes ({query_fields}) VALUES ({placeholders})
            ON CONFLICT (variant_id, sample_id)
            DO UPDATE SET {update_clause}
        """
        cursor.executemany(query, buffer)

    for variant_count, variant in enumerate(variants, start=1):
        variant_fields = [
            k for k in variant.keys() if k not in ("samples", "annotations")
        ]
        common_fields = [f for f in variants_local_fields if f in variant_fields]
        query_fields = ",".join(f"`{i}`" for i in common_fields)
        query_values = ",".join("?" for _ in common_fields)
        query_datas = [variant[i] for i in common_fields]

        # INSERT VARIANTS
        if RETURNING_ENABLE:
            query = f"""INSERT INTO variants ({query_fields}) VALUES ({query_values})
            ...
            variant_id = conn.execute(
                "SELECT id FROM variants WHERE chr=? AND pos=? AND ref=? AND alt=?",
                (chrom, pos, ref, alt),
            ).fetchone()[0]
        ...
        if "annotations" in variant:
            cursor.execute("DELETE FROM annotations WHERE variant_id=?", (variant_id,))
            for ann in variant["annotations"]:
                ann["variant_id"] = variant_id
                common_fields_ann = [f for f in annotations_local_fields if f in ann]
                fields_str = ",".join(f"`{f}`" for f in common_fields_ann)
                placeholders = ",".join("?" for _ in common_fields_ann)
                values = [ann[f] for f in common_fields_ann]
                cursor.execute(
                    f"INSERT OR REPLACE INTO annotations ({fields_str}) VALUES ({placeholders})",
                    values,
                )

        # BUFFER SAMPLES FOR BULK INSERT
        if "samples" in variant:
            for sample in variant["samples"]:
                if sample["name"] not in samples_map:
                    continue
                ...
                if sample["gt"] < 0:
                    # cursor.execute(
                    #     "DELETE FROM genotypes WHERE variant_id=? AND sample_id=?",
                    #     (sample["variant_id"], sample["sample_id"]),
                    # )
                    continue

                # Determine the columns for insertion in a stable order
                insert_fields = [f for f in samples_local_fields if f in sample]
                values = tuple(sample[f] for f in insert_fields)

                # If batch is empty, initialize batch fields
                if not genotype_buffer:
                    genotype_fields_batch = insert_fields
                elif insert_fields != genotype_fields_batch:
                    # Flush previous buffer if fields differ
                    _bulk_insert_genotypes(
                        cursor, genotype_buffer, genotype_fields_batch
                    )
                    genotype_buffer.clear()
                    genotype_fields_batch = insert_fields

                genotype_buffer.append(values)

                # Flush buffer if full
                if len(genotype_buffer) >= GENOTYPE_BATCH_SIZE:
                    _bulk_insert_genotypes(
                        cursor, genotype_buffer, genotype_fields_batch
                    )
                    genotype_buffer.clear()
    ...
    # Flush remaining buffered genotypes
    if genotype_buffer:
        _bulk_insert_genotypes(cursor, genotype_buffer, genotype_fields_batch)

    conn.commit()

    if progress_callback:
        progress_callback(
            f"{total_inserted} variant(s) inserted with {errors} error(s)"
        )
...
def search_samples(
    conn: sqlite3.Connection, name: str, families=[], tags=[], classifications=[]
):
...
def get_genotypes(
    conn, variant_id: int, fields: List[str] = None, samples: List[str] = None
):
...
def import_reader(
    ...
    if reader.filename:
        ...
        import_vcf = None
    insert_samples(
        conn, samples=reader.get_samples(), import_id=import_id, import_vcf=import_vcf
    )
